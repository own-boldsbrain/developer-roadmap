...
const REGISTRY_PATH = path.join(
  process.cwd(),
  '../.gemini/antigravity-ide/brain/f34d53a0-ec05-48c2-9c3e-acd9da77ad91/connection-test-candidates.json',
);
...
function resolveEndpoint(config?: EndpointConfig): {
...
function resolveCredential(config?: CredentialConfig): {
  configured: boolean;
  status: string;
} {
  if (!config || !config.required)
    return { configured: true, status: 'NOT_REQUIRED' };
...
async function runProbe(provider: ProviderCandidate): Promise<RunStats> {
  ...
  if (provider.capabilityProbe) {
    ...
    const capRes = await measureFetch(
      capUrl,
      { headers: authHeaders },
      timeoutMs,
    );
    ...
    if (capRes.error) {
      stats.g2Status =
        capRes.error.message === 'TIMEOUT' ? 'TIMEOUT' : 'UNREACHABLE';
      stats.overall = 'NOT_READY';
  ...
  if (provider.functionalProbe) {
    ...
    const fnRes = await measureFetch(
      fnUrl,
    ...
    if (expected && fnRes.text.includes(expected)) {
      ...
      stats.errors.push(
        `Expected semantic match failed. Received snippet: ${fnRes.text.slice(0, 100)}`,
      );
    }
  }
...
async function main() {
  ...
  await fs.copyFile(
    REGISTRY_PATH,
    path.join(OUTPUT_DIR, 'connection-test-candidates.json'),
  );
  await fs.writeFile(
    path.join(OUTPUT_DIR, 'provider-summary.json'),
  ...
  const resultsJsonl = Object.entries(results)
    .map(([id, stats]) => JSON.stringify({ providerId: id, ...stats }))
    .join('\n');
  await fs.writeFile(
    path.join(OUTPUT_DIR, 'provider-results.jsonl'),
    resultsJsonl,
  );
