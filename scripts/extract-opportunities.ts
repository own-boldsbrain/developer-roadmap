...
const limitStr = args.find(a => a.startsWith('--limit='))?.split('=')[1] || '5';
const provider = args.find(a => a.startsWith('--provider='))?.split('=')[1] || 'ollama';
const model = args.find(a => a.startsWith('--model='))?.split('=')[1] || 'qwen3.6:latest';
const selectionSeed = args.find(a => a.startsWith('--selection-seed='))?.split('=')[1] || 'default';
const isDryRun = args.includes('--dry-run');
const limit = parseInt(limitStr, 10);

const runId = `opp-extract-${new Date().toISOString().replace(/[:.T-]/g, '').slice(0, 14)}-${crypto.randomBytes(3).toString('hex')}`;
const RUN_DIR = path.join('.translation-control', 'opportunities', 'runs', runId);
...
let eligibleFiles = fs.readFileSync(INVENTORY_PATH, 'utf-8').split('\n').filter(Boolean).map(line => JSON.parse(line));
if (roadmapStr) {
...
const pseudoRandom = (i: number) => { const x = Math.sin(seedVal + i) * 10000; return x - Math.floor(x); };
eligibleFiles.sort((a, b) => pseudoRandom(a.sizeBytes) - pseudoRandom(b.sizeBytes));
...
async function queryModel(content: string, sourcePath: string) {
    ...
    if (provider === 'litellm') endpoint = 'http://localhost:4002/v1/chat/completions';
    
    const headers: any = { 'Content-Type': 'application/json' };
    if (provider === 'litellm') headers['Authorization'] = `Bearer ${process.env.LITELLM_API_KEY}`;

    const body = provider === 'ollama' ? {
        model: model,
        messages: [
            { role: 'system', content: systemPrompt },
            { role: 'user', content: `File: ${sourcePath}\\n\\nContent:\\n${content}` }
        ],
        format: 'json',
        stream: false,
...
async function run() {
    ...
    for (const entry of selectedFiles) {
        ...
        try {
            ...
            let parsed: any;
            try {
                const jsonStr = rawResponse.trim().replace(/^```json/, '').replace(/```$/, '').trim();
                parsed = JSON.parse(jsonStr);
            } catch(e) {
                logEvent('opportunity.parse_failed', fileId, 'ERROR', { raw: rawResponse });
                continue;
            }

            // Schema Strict Validation
            const allowedCategories = ["deprecated-technology", "outdated-version", "missing-link", "conceptual-gap", "insecure-practice", "duplicate-content", "inconsistency", "missing-a11y", "missing-observability", "missing-tests", "ia-improvement", "automation-opportunity", "ai-opportunity"];
            const allowedSeverities = ["critical", "high", "medium", "low"];
            const allowedHorizons = ["immediate", "30-days", "60-90-days", "strategic"];

            if (!parsed.findings || !Array.isArray(parsed.findings)) {
                appendLog(REJECTED_PATH, { fileId, error: "Missing findings array", raw: parsed });
                continue;
            }
            ...
            for (const f of parsed.findings) {
                ...
                for (const k of keys) {
                    if (!['category', 'severity', 'title', 'evidence', 'recommendation', 'confidence', 'timeHorizon'].includes(k)) {
                        unknownFields++;
                    }
                }

                // Strict validations
                if (!allowedCategories.includes(f.category) || !allowedSeverities.includes(f.severity) || !allowedHorizons.includes(f.timeHorizon)) {
                    appendLog(REJECTED_PATH, { fileId, finding: f, error: "Enum validation failed" });
                    continue;
                }
                
                if (typeof f.confidence !== 'number' || f.confidence < 0 || f.confidence > 1) {
                    appendLog(REJECTED_PATH, { fileId, finding: f, error: "Confidence out of bounds" });
                    continue;
                }
                
                if (!f.evidence || f.evidence.file !== sourcePath) {
                    appendLog(REJECTED_PATH, { fileId, finding: f, error: "Evidence file mismatch" });
                    continue;
                }

                // Evidence excerpt literal check
                if (!content.includes(f.evidence.excerpt)) {
                    appendLog(REJECTED_PATH, { fileId, finding: f, error: "Excerpt not literally in file" });
                    continue;
                }
                evidenceVerified++;
                ...
                if (runFindings.has(findingId)) {
                    duplicateFindingIds++;
                    appendLog(REJECTED_PATH, { fileId, finding: f, error: "Duplicate finding ID" });
                    continue;
                }
            ...
            // Isolate output
            const outPath = path.join(FILES_DIR, `${path.basename(sourcePath)}.opportunities.json`);
            fs.writeFileSync(outPath, JSON.stringify(parsed, null, 2));

            logEvent('opportunity.extraction_completed', fileId, 'INFO', { findings: parsed.findings.length });
            successCount++;
            
        } catch (error: any) {
            logEvent('opportunity.network_failed', fileId, 'ERROR', { message: error.message });
        }
    }

    fs.writeFileSync(METRICS_PATH, JSON.stringify({
        schemaPassRate: successCount > 0 ? (schemaPassRate / successCount) * 100 : 0,
        evidenceVerified: totalFindings > 0 ? (evidenceVerified / totalFindings) * 100 : 0,
        unknownFields,
        duplicateFindingIds,
